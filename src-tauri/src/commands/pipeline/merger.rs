...
/// Hard cap on outstanding (un-consumed) tokens. Bounds memory growth if a
/// caller issues tokens without consuming them. With ~5min TTL + GC-on-issue,
/// this is only reached under abuse / a bug.
const TOKEN_STORE_CAP: usize = 1024;
...
pub async fn run_merger_inner(input: MergerInput) -> MergerResult {
    ...
    if is_pr_mode {
        ...
        // `--json url -q .url` is gh's machine-readable contract — emits just the
        // URL on success, no banners. Plain stdout has historically had banners
        // ("Creating pull request for ...", draft warnings, etc.) that would
        // contaminate `pr_url` and break the modal's "Open PR" button.
        let res = run_cmd(
            "gh",
            &[
                "pr",
